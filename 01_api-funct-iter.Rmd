...
```{r}
raw_dataROMO<- httr::GET(url= "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=01&startYear=2010&endMonth=01&endYear=2021")
 
unpackedROMO<- httr:: content(raw_dataROMO, as = "text", encoding= "UTF-8")
final_ROMO<- jsonlite::fromJSON(unpackedROMO)
final_ROMO
...
test<-parkwide_visitation(year = 2010)
...
unit_visitation<- function(park,startmonth = "01",startyear,endmonth="12",endyear){
  unit_raw<- httr::GET(paste0(url="https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=" ,park,"&startMonth=",startmonth, "&startYear=",startyear,"&endMonth=", endmonth,"&endYear=",endyear))
                 
  unpacked_unit<- httr::content(unit_raw,as= "text", encoding = "UTF-8")
  final_unit<- jsonlite::fromJSON(unpacked_unit)
...
```{r}
unit_visitation(park= "ROMO",startmonth =  "01", startyear = "1990", endmonth = "01",endyear = "2021")

unit_visitation(park="EVER",startmonth= "01",startyear="1990", endmonth= "01", endyear= "2021")
unit_visitation(park="THRO",startmonth= "01",startyear="1990", endmonth= "01", endyear= "2021")
```
...
```{r}
unit_visitation(park="ROMO",startmonth= "01",startyear="1990", endmonth= "12", endyear= "2021")

unit_visitation(park="EVER",startmonth= "01",startyear="1990", endmonth= "12", endyear= "2021")
unit_visitation(park="THRO",startmonth= "01",startyear="1990", endmonth= "12", endyear= "2021")
```
...
outputfloop <- vector("list", length = length(parks))
for(i in 1:length(parks)){
  
  outputfloop[[i]] <-
    unit_visitation(park =parks[i],startmonth = 01,startyear = 1990,endmonth = 12, endyear = 2021)}
across_parks <-dplyr::bind_rows(outputfloop)
summary(across_parks)  
```
